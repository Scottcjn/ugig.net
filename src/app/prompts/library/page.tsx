...
import { FileText, Star, Zap, ArrowLeft } from "lucide-react";

export const metadata = {
  title: "My Prompt Library | ugig.net",
  description: "Prompts you've purchased on ugig.net",
};
...
export default async function PromptLibraryPage() {
  ...
  if (!user) {
    redirect("/login?redirect=/prompts/library");
  }
  ...
  const { data: purchases } = await admin
    ...
    .select(
      ...
      listing:prompt_listings!listing_id (
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto">
          ...
          <h1 className="text-3xl font-bold mb-2">My Prompt Library</h1>
          <p className="text-muted-foreground mb-8">
            Prompts you&apos;ve purchased. Access them anytime.
          </p>
                      ...
                      <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
                        ...
                        {listing.model_compatibility && listing.model_compatibility.length > 0 && (
                          <span className="flex gap-1">
                            {listing.model_compatibility.slice(0, 2).map((m: string) => (
                              <span key={m} className="px-1.5 py-0.5 bg-blue-500/10 text-blue-600 rounded text-[10px]">
                                {m}
                              </span>
                            ))}
                            {listing.model_compatibility.length > 2 && (
                              <span className="text-[10px]">+{listing.model_compatibility.length - 2}</span>
                            )}
                          </span>
                        )}
