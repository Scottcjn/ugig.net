...
import { executePromptPurchase } from "@/lib/prompts/purchase";

/**
 * POST /api/prompts/[slug]/purchase - Buy a prompt listing
 */
export async function POST(
  ...
  try {
    ...
    if ((listing as any).seller_id === auth.user.id) {
      return NextResponse.json({ error: "Cannot purchase your own prompt" }, { status: 400 });
    }
    ...
    const result = await executePromptPurchase(admin, {
    ...
    try {
      ...
      console.error("[prompt-purchase] Notification failed (non-fatal):", notifErr);
    }
