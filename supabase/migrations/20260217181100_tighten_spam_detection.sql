-- Tighten spam detection: lower entropy threshold, add mixed-case random pattern
...
BEGIN
  ...
  -- Mixed-case random: 14+ chars of only letters with lots of case switches = bot signup
  -- Real usernames rarely mix case randomly like "BlQyGebwabqMZMmdOp"
  IF uname ~ '^[a-zA-Z]{14,}$' THEN
    ...
    BEGIN
      ...
      END LOOP;
      -- High case-switch ratio = random generation (real names: 0-2 switches)
  ...
  -- Shannon entropy check (lowered threshold from 4.0 to 3.8, lowered min length from 12 to 10)
  IF length(uname) > 10 THEN
    ...
    FOR i IN 1..256 LOOP
      ...
      END IF;
    END LOOP;
    IF entropy > 3.8 THEN RETURN true; END IF;
  END IF;
